...
func NewFeishuAuthCodeIssueTokenRequest(authcode, state, domain string) *IssueTokenRequest {
	...
	req.Username = fmt.Sprintf("@%v", domain)
	return req
}
...
// 查询用户最近一次登陆记录
func NewQueryUserWebLastToken(uid int64) *QueryTokenRequest {
...
func GetAccessTokenFromHTTP(r *http.Request) string {
	...
	if err != nil {
		zap.L().Warnf("get tk from cookie: %v error, %v", ACCESS_TOKEN_COOKIE_KEY, err)
		return ""
	}
...
func (t *Status) BlockMessage() string {
	...
	return fmt.Sprintf("token blocked at %d, reason: %v", t.BlockAt, t.BlockReason)
}
